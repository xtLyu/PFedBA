...
def read_cifa_data():
    ...
    random.seed(1)
    np.random.seed(1)
    NUM_USERS = 10  # should be muitiple of 10 20;3
    NUM_LABELS = 5
    ...
    cifa_data = []
    for i in trange(10):   #进度条
        idx = cifa_data_label == i
        cifa_data.append(cifa_data_image[idx])

    # print("\nNumb samples of each label:\n", [len(v) for v in cifa_data])   #按类取出数据，分布存到数组里
    users_lables = []

    ###### CREATE USER DATA SPLIT #######
    # Assign 100 samples to each user   先给每个用户所有类别10张样本
    X = [[] for _ in range(NUM_USERS)]   #生成含有用户数的列表 [[], [], [], [], [], [], []]
    ...
    for user in range(NUM_USERS):
        for j in range(10):  # 3 labels for each users
            ...
            l = (user + j) % 10
            # print("L:", l)
            X[user] += cifa_data[l][idx[l]:idx[l] + 200].tolist()
            y[user] += (l * np.ones(200)).tolist()
            idx[l] += 200
    ...
    for user in trange(NUM_USERS):
        for j in range(NUM_LABELS):  # 4 labels for each users
            ...
            numran1 = random.randint(500, 550)
